  ...
  return (root, result) => {
    ...
    root.walk(node => {
      ...
      if (
        ...
        parent.document &&
        parent.first === node &&
        !/\n\s*$/.test(parent.raws.beforeStart)
      ) {
        openingBraceLevel = 0;
      } else {
        openingBraceLevel = nodeLevel;
      }

      const expectedOpeningBraceIndentation = _.repeat(
        indentChar,
        openingBraceLevel
      );
    ...
    function indentationLevel(node, level) {
      ...
      if (node.parent.type === "root") {
        return level + getRootBaseIndentLevel(node.parent, space);
      }
...
function getRootBaseIndentLevel(root, space) {
  const document = root.document;
  if (!document) {
    return 0;
  }
  return inferBaseIndentLevel(root, () => inferDocIndentSize(document, space));
}
...
function inferBaseIndentLevel(root, indentSize) {
  let baseIndentLevel = root.source.baseIndentLevel;
  ...
  const indents = source.match(/^[ \t]*(?=\S)/gm);
  if (indents) {
    baseIndentLevel = Math.min.apply(
      Math,
      indents.map(indent => {
        let tabCount = indent.match(/\t/g);
        tabCount = tabCount ? tabCount.length : 0;
        let spaceCount = indent.length - tabCount;
        spaceCount = spaceCount ? Math.round(spaceCount / indentSize()) : 0;
  ...
  root.source.baseIndentLevel = baseIndentLevel;
  return baseIndentLevel;
}
